...
#ifdef NGX_HTTP_PROXY_THRU
#include <ngx_http_proxy_thru.h>
#endif
...
static ngx_int_t ngx_http_upstream_process_header_line(ngx_http_request_t *r,
                                                       ngx_table_elt_t *h,
                                                       ngx_uint_t offset);
static ngx_int_t ngx_http_upstream_process_multi_header_lines(
...
static void ngx_http_upstream_conn_established_handler(ngx_http_request_t *r,
                                                       ngx_http_upstream_t *u);

static void ngx_http_upstream_tunnel_established_handler(ngx_http_request_t *r,
                                                         ngx_int_t rc);

static char *ngx_http_upstream_merge_loc_conf(ngx_conf_t *cf, void *parent,
                                              void *child);

void ngx_http_ntls_callback(ngx_connection_t* conn);

extern ngx_module_t ngx_http_proxy_thru_module;
#endif
...
static ngx_http_upstream_header_t ngx_http_upstream_headers_in[] = {
    ...
    {ngx_string("Refresh"), ngx_http_upstream_process_header_line,
    ...
    {ngx_string("Accept-Ranges"), ngx_http_upstream_ignore_header_line, 0,
    ...
    {ngx_string("Content-Encoding"), ngx_http_upstream_ignore_header_line, 0,
     ngx_http_upstream_copy_header_line,
...
#if NGX_HTTP_PROXY_THRU
    ngx_http_upstream_merge_loc_conf
#else
...
static void ngx_http_upstream_connect(ngx_http_request_t *r,
  ...
  u->write_event_handler =
      ngx_http_upstream_conn_established_handler; // ngx_http_upstream_send_request_handler;
#else
...
static ngx_int_t ngx_http_upstream_ssl_certificate(ngx_http_request_t *r,
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log, 0, "http upstream ssl key: \"%s\"",
                 key.data);
...
static void ngx_http_upstream_conn_established_handler(ngx_http_request_t *r,
  ...
  ngx_http_proxy_thru_loc_conf_t *ptpclcf =
      ngx_http_get_module_loc_conf(r, ngx_http_proxy_thru_module);
  if (ptpclcf->url.url.len == 0) {
    u->write_event_handler = ngx_http_upstream_send_request_handler;
...
static void ngx_http_upstream_tunnel_established_handler(ngx_http_request_t *r,
  ...
  r->upstream->write_event_handler = ngx_http_upstream_send_request_handler;
  r->upstream->read_event_handler = ngx_http_upstream_process_header;
  ngx_http_upstream_send_request_handler(r, r->upstream);
}
#endif
...
static void ngx_http_upstream_thread_event_handler(ngx_event_t *ev) {
  ...
  if (ev->timedout) {
    ngx_log_error(NGX_LOG_ALERT, c->log, 0, "thread operation took too long");
    ev->timedout = 0;
    return;
  }
...
static ngx_int_t ngx_http_upstream_process_content_length(ngx_http_request_t *r,
  ...
  if (u->headers_in.content_length) {
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                  "upstream sent duplicate header line: \"%V: %V\", "
                  "previous value: \"%V: %V\"",
                  &h->key, &h->value, &u->headers_in.content_length->key,
...
static ngx_int_t ngx_http_upstream_process_last_modified(ngx_http_request_t *r,
  ...
  if (u->headers_in.last_modified) {
    ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
                  "upstream sent duplicate header line: \"%V: %V\", "
                  "previous value: \"%V: %V\", ignored",
                  &h->key, &h->value, &u->headers_in.last_modified->key,
...
static ngx_int_t ngx_http_upstream_process_expires(ngx_http_request_t *r,
  ...
  if (u->headers_in.expires) {
    ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
                  "upstream sent duplicate header line: \"%V: %V\", "
                  "previous value: \"%V: %V\", ignored",
                  &h->key, &h->value, &u->headers_in.expires->key,
...
static ngx_int_t ngx_http_upstream_process_accel_expires(ngx_http_request_t *r,
  ...
  if (u->headers_in.x_accel_expires) {
    ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
                  "upstream sent duplicate header line: \"%V: %V\", "
                  "previous value: \"%V: %V\", ignored",
                  &h->key, &h->value, &u->headers_in.x_accel_expires->key,
...
static ngx_int_t ngx_http_upstream_trailer_variable(
  ...
  return ngx_http_variable_unknown_header(
      r, v, (ngx_str_t *)data, &r->upstream->headers_in.trailers.part,
...
static ngx_int_t ngx_http_upstream_cookie_variable(ngx_http_request_t *r,
  ...
  if (ngx_http_parse_set_cookie_lines(r, r->upstream->headers_in.set_cookie, &s,
                                      &cookie) == NULL) {
    v->not_found = 1;
    return NGX_OK;
  }
...
#if NGX_HTTP_PROXY_THRU
  uscf->proxy_thru = flags & NGX_HTTP_UPSTREAM_PROXY_THRU;
#endif
...
static char *ngx_http_upstream_merge_loc_conf(ngx_conf_t *cf, void *parent,
                                              void *child) {
  ngx_http_proxy_thru_loc_conf_t *ptpclcf =
      ngx_http_conf_get_module_loc_conf(cf, ngx_http_proxy_thru_module);
  if (ptpclcf->url.url.len == 0) {
    return NGX_CONF_OK;
  }
  ...
  for (ngx_uint_t i = 0; i < umcf->upstreams.nelts; i++) {
    ngx_http_upstream_srv_conf_t *upstream =
        ((ngx_http_upstream_srv_conf_t **)umcf->upstreams.elts)[i];
    if (upstream->proxy_thru) {
      ngx_http_upstream_init_proxy_thru(cf, upstream, &url);
    }
  }
